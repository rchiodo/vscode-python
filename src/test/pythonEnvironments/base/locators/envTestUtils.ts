...
export function assertBasicEnvsEqual(actualEnvs: BasicEnvInfo[], expectedEnvs: BasicEnvInfo[]): void {
    actualEnvs = actualEnvs.sort((a, b) => a.executablePath.localeCompare(b.executablePath)).map(c => {
        return {...c, executablePath: c.executablePath.toLowerCase()}
    });
    expectedEnvs = expectedEnvs.sort((a, b) => a.executablePath.localeCompare(b.executablePath)).map(c => {
        return {...c, executablePath: c.executablePath.toLowerCase()}
    });;
    assert.deepStrictEqual(actualEnvs.length, expectedEnvs.length, 'Number of envs');
    zip(actualEnvs, expectedEnvs).forEach((value) => {
